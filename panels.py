"""Panel UI for Adobe Acrobat Sign Connector following UI_INTERFACE_STANDARD.md and AUTH_AND_CREDENTIALS_STANDARD.md."""
...
def _settings_button() -> ui.UINode:
    return ui.Button(
        ...
        size="sm",
        icon="settings",
        on_click=ui.Call("__panel__adobe_acrobat_sign_settings")
    )

def _help_modal() -> ui.UINode:
    return ui.Modal(
        trigger=ui.Button("How do I set this up?", variant="ghost", size="sm"),
        title="Connecting Adobe Acrobat Sign",
        children=[
            ui.Text(
                "1. Sign in to your Adobe Acrobat Sign account and navigate to API/Integration or OAuth settings.\n2. Choose your preferred authentication method (OAuth SSO, API Key / Personal Token, or Client Credentials / Service Account).\n3. Authorize or enter your credentials above and click Connect.",
                variant="body"
            )
        ]
    )

@ext.panel("adobe_acrobat_sign_sidebar", slot="left")
async def adobe_acrobat_sign_sidebar(ctx, **kwargs) -> ui.UINode:
    return ui.Stack(
        direction="v",
        gap=3,
        align="stretch",
        children=[
            ui.Text("Adobe Acrobat Sign", variant="heading"),
            ui.Stack(
                direction="v",
                gap=1,
                align="stretch",
                children=[
                    ui.Text("Manage your Adobe Acrobat Sign connections and integrations.", variant="caption"),
                ]
            ),
            ui.Divider(),
            ui.Stack(
                ...
                children=[
                    ui.Button(
                        "Sign in with Adobe Acrobat Sign (OAuth / SSO)",
                        variant="primary",
                        size="sm",
                        icon="login"
                    ),
                    ...
                    ui.Form(
                        submit_label="Connect Adobe Acrobat Sign",
                        action=ui.Call("connect_adobe_acrobat_sign"),
                        children=[
                            ui.Stack(
                                direction="v",
                                gap=2,
                                ...
                                children=[
                                    ...
                                    ui.Stack(
                                        ...
                                        children=[
                                            ...
                                            ui.Input(param_name="label", placeholder="e.g. Production Adobe Acrobat Sign"),
                                        ]
                                    ),
                                    ui.Stack(
                                        direction="v",
