"""Resource handlers for Adobe Acrobat Sign Connector."""
...
@chat.function(
    "list_agreements",
    "List agreements or signature requests in Adobe Acrobat Sign.",
    action_type="read",
    chain_callable=True,
    event="adobe-acrobat-sign-connector.list_agreements",
...
async def list_agreements(params: ListAgreementsParams, ctx) -> ActionResult[AgreementList]:
    try:
        ...
        return ActionResult.success(AgreementList(agreements=agreements, total=len(agreements)), summary=f"Retrieved {len(agreements)} agreements from Adobe Acrobat Sign.")
    except Exception as e:
        return ActionResult.error(f"Error listing agreements: {e}")

@chat.function(
    "get_agreement",
    "Get details of one agreement or signature request in Adobe Acrobat Sign.",
    action_type="read",
    chain_callable=True,
    event="adobe-acrobat-sign-connector.get_agreement",
...
@chat.function(
    ...
    "Send a new document for signature via Adobe Acrobat Sign.",
    action_type="write",
    chain_callable=True,
    event="adobe-acrobat-sign-connector.create_signature_request",
...
@chat.function(
    ...
    "Cancel or void an in-flight signature request in Adobe Acrobat Sign.",
    action_type="destructive",
    chain_callable=True,
    event="adobe-acrobat-sign-connector.cancel_signature_request",
...
@chat.function(
    "audit_esign_health",
    "Audit in-flight signature requests, turnaround rates, and expired agreements in Adobe Acrobat Sign.",
    action_type="read",
    chain_callable=True,
    event="adobe-acrobat-sign-connector.audit_esign_health",
    effects=["read:audit"],
...
async def audit_esign_health(params: NoParams, ctx) -> ActionResult[ESignAuditReport]:
    try:
        ...
        rep = ESignAuditReport(
            ...
            summary=f"Adobe Acrobat Sign health audit: {len(agreements)} total tracked agreements, {pending} pending signatures, {completed} completed."
        )
