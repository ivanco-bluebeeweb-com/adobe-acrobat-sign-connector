...
async def connect_adobe_acrobat_sign(ctx, params: ConnectParams) -> ActionResult[ConnectionRecord]:
...
async def disconnect_adobe_acrobat_sign(ctx, params: ConnectionIdParams) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
    if not conns:
